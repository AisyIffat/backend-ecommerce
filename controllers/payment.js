...
const verifyPayment = async (
  ...
  // 2. compare the generateed x-signature with the one from billplz
  ...
  if ( billplz_paid === "true" ) {
    // if the billplz_paid is equal to true, then payment is successful
    selectedOrder.status = "paid";
    selectedOrder.paid_at = billplz_paid_at; // when the order is paid
  } else {
    // if the billplz_paid is not true, then payment is failed
